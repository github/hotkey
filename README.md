...
9. `"Shift"` should be included if it would be held and the key is uppercase: ie, `Shift+A` not `a`. Note however that MacOS outputs lowercase keys when `Command+Shift` is held; see 6.3 above.
